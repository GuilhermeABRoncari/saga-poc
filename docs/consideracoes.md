...
- **Latência fim-a-fim (T6.2 — 1000 sagas sequenciais):** RabbitMQ p50=21ms / p99=22ms (max 25ms, distribuição apertada) vs Temporal p50=60ms / p99=351ms (distribuição bimodal, ~16x mais lento em p99). RabbitMQ ~6.2x mais throughput sequencial. _Re-medição em 4.3 pendente; espera-se p99 ainda menor._
...
**Esse não é um leak — é storage de audit trail durável.** O Postgres do Temporal só limpa após o retention period (default: 7 dias para Completed). É exatamente o que torna `tctl workflow show` instantâneo dias depois (ver T3.4).
...
- 4.3M × 5 KB = **~21 GB/mês de history** no Postgres do Temporal.
