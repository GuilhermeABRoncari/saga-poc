...
- **Como executou:**
  ...
  - RabbitMQ: disparar saga com `SLOW_RESERVE_STOCK=15`. Durante o sleep do step 0, editar `ActivateStoreSaga::definition()` inserindo novo step `audit_log` entre `reserve_stock` e `charge_credit`, copiar para container, restartar orchestrator.
- **Resultado Temporal:**
...
- [!] **Executado em 2026-04-29 — não reproduziu duplicação na janela testada.**
- **Como executou:**
...
- **Resultado RabbitMQ:**
  ...
  - Tempo total até 100 COMPLETED: **2076ms** (~48 sagas/s)
  - Tempo após fire: **1763ms**
  - Falhas: 0
  - Memória total stack após teste: **~187 MB** (rabbitmq 161 + orchestrator 10 + service-a 8 + service-b 8)
- **Resultado Temporal:**
...
- **Notas:**
  ...
  - Em produção, "RabbitMQ caído" raramente acontece em cluster com 3 nodes + quorum queues, mas SDK do `vladimir-yuldashev/laravel-queue-rabbitmq` (real opção que adotaríamos) tem reconexão automática. A lib que fizemos no PoC não tem.
...
- [⏭] **Não executado nesta sessão. Estimativa preservada como prevista.**
...
- **O que mudou na nossa visão depois dos testes T1-T2.3:** a estimativa original de "3-5 dias para mínimo aceitável" em [`findings-rabbitmq.md`](./findings-rabbitmq.md) §4 **continua válida** — a complexidade não está em construir cada peça, está em integrá-las e descobrir os edge cases (cardinality de labels, granularidade de buckets, semântica de "saga em andamento" sem race contra DB). Em Temporal Web UI esses 1.5 dias custam zero.
- **Por que importa o número:** o "tempo até observabilidade decente" é o critério que mais pesa contra RabbitMQ no padrão organizacional. Não basta ter ferramentas (Prometheus + Grafana são free); o trabalho de instrumentar + manter dashboards é recorrente, e a lib precisa não regredir nos counters cada vez que muda.
...
- [x] **Implementado e validado em 2026-04-29.**
- **Como executou:**
...
- **Análise:**
  - **Custo de implementação ficou parecido (40 LOC ambos)** porque escolhemos a abordagem "polling de status" nos dois — é simples, robusta e não exige Prometheus/AlertManager.
  - **A vantagem que o Temporal mantém está em "out of the box"**: o workflow já fica `Failed` automaticamente quando compensação falha, sem precisar mudar código nosso. No RabbitMQ tivemos que adicionar emissão de `compensation.failed` na ServiceWorker + handler no Orchestrator que seta `FAILED` (~12 LOC extras feitas em T1.2). Sem isso, o alerter não tem nada para detectar.
...
- **Notas:**
  ...
  - O argumento que **se mantém forte** é: Temporal entrega a `ExecutionStatus='Failed'` AUTOMATICAMENTE para QUALQUER tipo de falha (timeout, error, panic, retries esgotados) sem código nosso; RabbitMQ exige que cada caminho de falha seja explicitamente convertido em `status='FAILED'` na lib. Quanto mais caminhos de falha, mais código + mais chance de esquecer um.
...
- [⏭] **Não executável dentro do escopo da PoC.**
...
- **Sugestão de checklist de perguntas para o dev:**
  ...
  - Onde eu olho pra ver o que aconteceu na saga 1234?
- **Resultado RabbitMQ:**
- **Resultado Temporal:**
...
- [⏭/~] **Não executado totalmente — purgar cache Docker e re-baixar/re-compilar imagens consumiria ~30-40min de CI/eng. Estimativa baseada em medições parciais e dados do [`findings-temporal.md`](./findings-temporal.md) §1.**
...
- **Comparação direta:**
  ...
  | Timestamps de cada evento | ⚠️ via logs (efêmeros) | ✅ history nativo |
  ...
  | Replay programático | ❌ | ✅ |
  | Search/filter | ⚠️ SQL custom | ✅ List Filter Query syntax |
  | Tempo prático para postmortem | 2-15min (varia muito) | 30s-1min |
- **Veredito:** Temporal vence claramente em **profundidade de informação** e **DX**. RabbitMQ entrega o básico rápido (status+path) mas depende de logs e instrumentação extra para ir além. Para 4 sistemas com 4 times diferentes investigando incidentes, a diferença vira **dias de produtividade ao longo de meses**.
- **Notas:**
...
- [~] **Executado parcialmente em 2026-04-29.** Não foi possível encher 45 GB do volume host com segurança; substituído por simulações análogas de "write failure" no SQLite.
- **Tentativas:**
...
- **Veredito:** lib RabbitMQ atual **não é robusta a falhas de storage**. Requer pelo menos:
  ...
  3. Modo "degradado" que parou de aceitar novas sagas quando storage indisponível, em vez de aceitar e perder. (~20 LOC)
...
- **Notas:** este é mais um item onde a "infraestrutura grátis" do Temporal aparece. Para 4 sistemas, ter timeouts explícitos e diferenciáveis em todos os steps é importante para postmortem e SLOs. Em RabbitMQ, isso teria que ser disciplina + biblioteca de helpers.
...
- **Resultado RabbitMQ (versionamento manual — não implementado, estimado):**
  - Para alcançar o mesmo comportamento (sagas em voo continuam fluxo antigo, sagas novas usam fluxo novo), o trabalho **não previsto pela lib atual** envolve:
    ...
    3. Saga base (`Mobilestock\Saga\Saga`): expor método `currentVersion(): int` (default 1) e permitir override por saga concreta. (~5 LOC)
...
- **Comparação direta:**
  ...
  - **RabbitMQ:** falha SILENT, marca SUCESSO falso, state corrompido. Em produção: estoque reservado 2x, pagamento NUNCA cobrado, pedido marcado completo. **Pior cenário possível para padrão organizacional.**
...
- [⏭] **Não executado nesta sessão.** Razão: requer criar conta na Temporal Cloud e configurar credenciais — fora do escopo da execução automatizada local.
- **Como conduzir quando puder:**
  ...
  5. Calcular: actions × custo unitário × volume estimado (4 sistemas × 100 sagas/min × 60 × 24 × 30 = ~17M sagas/mês).
- **O que esperar (estimativa baseada na documentação Temporal):**
  ...
  - Comparado a self-host EKS estimado de $250-500/mês infra + 1-2 dias eng/mês.
- **Implicação:** para o volume esperado, **Temporal Cloud fica caro rapidamente** (~$58k/ano). Self-host EKS é a opção financeira sensata para escala >10M actions/mês. A vantagem operacional do Cloud aparece só durante adoção (primeiros 6-12 meses).
