...
| **PoC**                            | **Proof of Concept** — protótipo concreto construído para validar uma hipótese antes de adotar uma tecnologia em produção. Aqui temos `saga-rabbitmq/`, `saga-temporal/`, `saga-rabbitmq-coreografado/` e `saga-step-functions/`.                 | Todos os documentos.                            |
...
| **Temporal**       | Plataforma open-source de durable execution para workflows de longa duração. Mantida por Temporal Inc; SDK PHP por Spiral Scout.                                     | `saga-temporal/`, `findings-temporal.md`.   |
| **Step Functions** | Serviço AWS managed para orquestrar workflows como state machines em ASL.                                                                                            | `recomendacao-saga.md` §2.3, 3ª PoC.        |
...
| **ASL**            | **Amazon States Language** — DSL em JSON usada por Step Functions para definir workflows (states, transitions, retry, catch).                                        | 3ª PoC.                                     |
...
| **Laravel**        | Framework PHP usado nos serviços avaliados pelo estudo.                                                                                                              | Todos os documentos.                        |
...
| **Event sourcing**          | Padrão onde estado é derivado de uma sequência append-only de eventos. Temporal armazena history events em Postgres/MySQL/Cassandra (MariaDB não suportado) e reconstroi state por replay.                 | `findings-temporal.md`.                                 |
...
| **Docker Swarm**     | Orquestrador de containers da Docker Inc. Considerado como ponto de partida em cenários de stack híbrida Swarm/Kubernetes.                                                                                                    | `recomendacao-saga.md` §1.1.  |
...
| **Aurora**           | Família AWS de bancos managed (Aurora Postgres / Aurora MySQL / Aurora MariaDB-compatible). Para Temporal self-host em EKS, opções viáveis são **Aurora Postgres ou Aurora MySQL** (Temporal não suporta MariaDB — `findings-temporal.md` §2.2.6). | `recomendacao-saga.md`.       |
...
| **Bref**             | Custom runtime PHP para AWS Lambda. Permite rodar Laravel em Lambda.                                                                                                                                                          | 3ª PoC.                       |
| **LocalStack**       | Emulador local de serviços AWS (Step Functions, SQS, Lambda etc). Permite PoC sem custo.                                                                                                                                      | 3ª PoC.                       |
...
| Termo                                           | Significado                                                                                                              |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **PDO**                                         | **PHP Data Objects** — abstração de DB do PHP. Usada nas PoCs RabbitMQ para SQLite/MariaDB.                              |
...
- **Ao ler docs antigos:** se encontrar sigla desconhecida, verificar aqui antes de buscar fora do repositório.
- **Para revisão por leitores externos:** este é o ponto de partida — entendendo este glossário, dá para ler qualquer outro documento do estudo sem dependência externa.
